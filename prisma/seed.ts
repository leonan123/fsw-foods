...
const deleteAllRegisters = async () => {
  ...
  await Promise.all([
    prismaClient.restaurant.deleteMany(),
...
const main = async () => {
  ...
  await Promise.all([
    createBurgers(desertsCategory.id, juicesCategory.id),
    createPizzas(desertsCategory.id, juicesCategory.id),
    createJapanese(desertsCategory.id, juicesCategory.id),
    createBrazilian(desertsCategory.id, juicesCategory.id),
  ]);
};
